...
func Test0xFE(t *testing.T) {
	c := mockCPU()
	c.registers.AF.low = 0x05
	c.mmu.WriteBytes([]byte{0xFE, 0x06}, 0)

	c.Step()
	testhelpers.AssertByte(t, c.registers.AF.low, 0x05)
	assertFlagState(t, "-NHC", c.registers.flagToString())
}
